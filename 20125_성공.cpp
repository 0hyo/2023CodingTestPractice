...
int main()
{
    ...
    int head[2], left_arm = 0, right_arm = 0, left_leg = 0, right_leg = 0, waist = 0;
// 머리 찾기
    i = 1, j = 1;
...
//왼팔 길이
    i = head[0] + 1, j = head[1] - 1;   // 왼팔 시작 위치
    while (1)
    {
        if(a[i][j--] != '*')
        {
...
//오른팔 길이
    i = head[0] + 1, j = head[1] + 1;   // 오른팔 시작 위치
    while (1)
    {
        if(a[i][j++] != '*')
        {
...
//왼다리 길이
    i = head[0] + 2 + waist, j = head[1] - 1;   // 왼다리 시작 위치
    while (1)
    {
        if(a[i][j] != '*')
        {
...
//오른다리 길이
    i = head[0] + 2 + waist, j = head[1] + 1;   // 오른다리 시작 위치
    while (1)
    {
        if(a[i++][j] != '*')
        {
