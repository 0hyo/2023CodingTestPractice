...
N장의 카드가 좌우 일렬로 놓여있다. 각 카드에는 하나의 정수가 적혀있는데, 왼쪽에서 
i번째 카드에 적혀있는 정수는 
A_i다. (1 ≤ i ≤ N)

여러분은 
N장의 카드 중 몇 장을 골라 제거할 수 있다. 이때, 제거되지 않은 카드의 순서는 유지된다.

예를 들어, 
N = 5, A = [3, 1, 4, 1, 5]라고 하자.
...
예를 들어, 
N = 8, 
A = [6, 1, 2, 3, 2, 4, 5, 10]라고 하자. 여러분이 첫 번째, 다섯 번째, 여덟 번째 카드를 제거하면, 남은 카드들에 적혀있는 수들은 차례대로 
1, 2, 3, 4, 5가 된다. 이때, 모든 남은 카드는 제자리 카드가 되며, 따라서 제자리 상태가 되었다.
