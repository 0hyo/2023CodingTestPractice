...
using namespace std;
/**
 * @brief 참고자료 : https://gusdnr69.tistory.com/248 
문제
...
void dfs()
{
    ...
    // queue에 pair을 삽입하는 방법으로 둘중 아무거나 써도 됨.
    ...
    while (!pq.empty())
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            {
                if (total_rupee[ny][nx] > total_rupee[y][x] + rupee[ny][nx] )
                {
                    total_rupee[ny][nx] = total_rupee[y][x] + rupee[ny][nx];
...
int main()
{
    ...
    while (1)
    {
        ...
        total_rupee[0][0] = rupee[0][0];
        dfs();
        
        

        cout << "\nProblem " << cnt++ <<": " <<total_rupee[N - 1][N - 1] <<"\n";
    }
