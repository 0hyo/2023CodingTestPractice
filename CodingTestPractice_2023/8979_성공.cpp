...
{
    int N, K;  cin >> N >> K;
    int country[1001][3], X;
    for (int cnt_i = 1; cnt_i <= N; cnt_i++)
    {
        cin >> X >> country[X][0] >> country[X][1] >> country[X][2];
    }
    int standard[3] = {country[K][0], country[K][1], country[K][2]};
    int rank = 1;
    for (int cnt_i = 1; cnt_i <= N; cnt_i++)
    {
        if (standard[0] < country[cnt_i][0])
        {
            rank++;   
        }
        else if (standard[0] == country[cnt_i][0] && standard[1] < country[cnt_i][1])
        {
            rank++;
        }
        else if (standard[0] == country[cnt_i][0] && standard[1] == country[cnt_i][1] && standard[2] < country[cnt_i][2])
        {
            rank++;
        }
    }
