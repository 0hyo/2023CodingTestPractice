 ...
 * 
 */
void print_Switch(int* Switch, int N)
{
    // cout <<"\n";
...
int main()
{
    int N, M, numbers_of_people, gender, number;
    int Switch[101] = {-1, };
    cin >> N;
    ...
    cin >> numbers_of_people;
    for (int cnt_i = 0; cnt_i < numbers_of_people; cnt_i++)
    {
        cin >> gender >> number;
        // print_Switch(Switch, N);
        if(gender == 1)
        {
            int tmp = number;
            while (tmp <= N)
        ...
        else if(gender == 2)
        {
            ...
            while(1)
            {
                if(Switch[tmp1 - 1] != Switch[tmp2 + 1])
                    break;
                if(tmp1 == 1 || tmp2 == N)
                    break;
                tmp1--;     tmp2++; 
            }
            for (int cnt_j = tmp1; cnt_j <= tmp2; cnt_j++)
            {
                Switch[cnt_j] = !Switch[cnt_j];
            }
