...
위의 상태에서 
K = 1인 경우를 생각해보자. 이 경우 모든 사람은 자신과 인접한 햄버거만 먹을 수 있다. 
...
첫 줄에 두 정수 
N과 
K가 있다. 그리고 다음 줄에 사람과 햄버거의 위치가 문자 P(사람)와 H(햄버거)로 이루어지는 길이 
N인 문자열로 주어진다.
...
int main()
{
    ...
    for (int cnt_i = 0; cnt_i < N; cnt_i++)
    {
        if (A[cnt_i] == 'P')
        {
            for (int cnt_j = max(0, cnt_i - K); cnt_j < cnt_i; cnt_j++)
            {
                if (A[cnt_j] == 'H')
                {
                    cnt++; 
            ...
            for (int cnt_j = cnt_i + 1; cnt_j <= min(cnt_i + K, N - 1); cnt_j++)
            {
                if (A[cnt_i] != 'P')
                    break;
                if (A[cnt_j] == 'H')
